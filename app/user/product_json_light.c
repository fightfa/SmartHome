...
int ICACHE_FLASH_ATTR product_json_ask_set(char * _buf, uint64_t _msg_id, uint8_t _operation_status, uint8_t _power_switch, uint8_t alpha)
{
    my_cJSON *root;
    ...
    if(!root)
    {
        PRODUCT_JSON_DEBUG("get root faild !\n");
        return -1;
    }
    ...
    {
        PRODUCT_JSON_DEBUG("status error, user default ! \r\n");
...
//        os_memcpy(_buf, s, os_strlen(s));
        os_strncpy(_buf, s, os_strlen(s));
        PRODUCT_JSON_DEBUG("product_json_ask_set:\r\n %s\r\n",_buf);
...
//TODO
int ICACHE_FLASH_ATTR product_json_ask_get(char * _buf, uint64_t _msg_id, char *_user, uint8_t _operation_status, uint8_t _power_switch, uint8_t _alpha)
{
    my_cJSON *root;
    my_cJSON * js_services ;
    my_cJSON * js_operation_status ;
...
int ICACHE_FLASH_ATTR product_json_notify(char * _buf, uint8_t _operation_status, uint8_t _power_switch, uint8_t _alpha)
{
    my_cJSON *root;
    my_cJSON * js_services ;
    my_cJSON * js_operation_status ;
...
//        os_memset(_buf, 0, sizeof(_buf));
        os_memcpy(_buf, s, os_strlen(s));
        PRODUCT_JSON_DEBUG("product_json_notify:\r\n %s\r\n",_buf);
        os_free(s);
    }
    my_cJSON_Delete(root);
    return 0;
